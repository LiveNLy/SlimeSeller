...
namespace BasketsStuff
{
    ...
    public class PositionSetter : MonoBehaviour
    {
        ...
        public void SetPosition(float zCorrecter, Slime slime, BasketMover mover)
        {
            ...
            _zCor = zCorrecter;
            Collider[] colliders = Physics.OverlapSphere(new Vector3(_position.x, _position.y, _position.z + _zCor - _sphereCorrector), _sphereRadius);
            ...
            foreach (Collider collider in colliders)
            {
                ...
                if (collider.TryGetComponent<SpawnPoint>(out SpawnPoint point))
                {
                    if (_validPoints.Contains(point))
                    {
                        ...
                        _position += new Vector3(0, 0, -0.5f);
                    }
                }
